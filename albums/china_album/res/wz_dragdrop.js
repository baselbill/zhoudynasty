...
{
	if (dd.obj.name != 'track') return;
		
	var percentage = Math.min(1, dd.e.x / maxThumbX);
	var moveToX = dd.e.x > maxThumbX ? maxThumbX : (dd.e.x < minThumbX ? minThumbX : dd.e.x);
	dd.elements.thumb.moveTo(moveToX, dd.elements.thumb.y);
	scrollDIVToPercentage(percentage);
}
...
{
	if (dd.obj.name != 'thumb') return;
	
	var percentage = Math.min(1, (dd.obj.x-1) / maxThumbX);
	scrollDIVToPercentage(percentage);
}
