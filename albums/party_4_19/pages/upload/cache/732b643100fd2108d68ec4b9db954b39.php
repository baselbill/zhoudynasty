...
<div class="white_box"> 
  ...
  <table style="width: 100%">
    ...
    ';for($i=0; $i<count($_vars['users']); $i++){print'    <tr style="background-color: ' . (($i & 1) ? 'white' : '#F4F4F4') . '">  
      ...
      <td>' . (isset($_vars['users'][$i]['approved']) ? $this->yes_or_no(@$_vars['users'][$i]['approved']) : 'yes_or_no') . '</td>     
      <td>' . (isset($_vars['users'][$i]['closed']) ? $this->yes_or_no(@$_vars['users'][$i]['closed']) : 'yes_or_no') . '</td>
    </tr>
    ';}print' 
  </table>
